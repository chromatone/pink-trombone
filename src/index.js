...
var Glottis = {
	...
	handleTouches: function () {
		...
		if (this.touch != 0) {
			...
			var semitone =
				(this.semitones * local_x) / this.keyboardWidth + 0.5;
	...
	getNoiseModulator: function () {
		var voiced =
			...
			Math.max(
				0,
				Math.sin(
					(Math.PI * 2 * this.timeInWaveform) / this.waveformLength
				)
			);
		//return 0.3;
		return (
	...
	normalizedLFWaveform: function (t) {
		let output
		if (t > this.Te) {
			output = (-Math.exp(-this.epsilon * (t - this.Te)) + this.shift) /
				this.Delta;
		}
		else {
...
var Tract = {
	...
	calculateReflections: function () {
		...
		this.newReflectionRight =
			(2 * this.A[this.noseStart + 1] - sum) / sum;
	...
	runStep: function (glottalOutput, turbulenceNoise, lambda) {
		...
		this.junctionOutputL[this.n] =
			this.R[this.n - 1] * this.lipReflection;

		for (var i = 1; i < this.n; i++) {
			var r =
				this.reflection[i] * (1 - lambda) +
				this.newReflection[i] * lambda;
		...
		var r =
			this.newReflectionLeft * (1 - lambda) +
			this.reflectionLeft * lambda;
		...
		r =
			this.newReflectionNose * (1 - lambda) +
			this.reflectionNose * lambda;
		...
		for (var i = 1; i < this.noseLength; i++) {
			var w =
				this.noseReflection[i] * (this.noseR[i - 1] + this.noseL[i]);
		...
		for (var i = 0; i < this.noseLength; i++) {
			...
			this.noseR[i] = Math.clamp(
				this.noseJunctionOutputR[i] * 0.999,
				-1,
				1
			);
...
var TractUI = {
	...
	moveTo: function (i, d) {
		var angle =
			this.angleOffset +
			(i * this.angleScale * Math.PI) / (Tract.lipStart - 1);
		var wobble =
	...
	draw: function () {
		...
		for (
			var i = Tract.noseStart + Math.ceil(velumAngle) - 2;
			i < Tract.n;
			i++
		)
			this.lineTo(i, 0);
	...
	drawBackground: function () {
		...
		this.drawText(
			Tract.n * 0.44,
			-0.28,
			IMAGINARY.i18n.t("SOFT_PALATE_1")
		);
		this.drawText(
			Tract.n *
			(0.47 +
		...
		this.drawText(
			Tract.n * 0.74,
			-0.28,
			IMAGINARY.i18n.t("HARD_PALATE_1")
		);
		this.drawText(
			Tract.n *
			(0.77 +
	...
	drawTongueControl: function () {
		...
		var angle =
			this.angleOffset +
			(this.tongueIndex * this.angleScale * Math.PI) /
			(Tract.lipStart - 1);
	...
	setRestDiameter: function () {
		for (var i = Tract.bladeStart; i < Tract.lipStart; i++) {
			...
			var curve =
				(1.5 - fixedTongueDiameter + this.gridOffset) * Math.cos(t);
	...
	handleTouches: function () {
		if (this.tongueTouch != 0 && !this.tongueTouch.alive)
			this.tongueTouch = 0;
		...
		if (this.tongueTouch != 0) {
			...
			fromPoint =
				Math.pow(fromPoint, 0.58) -
				0.2 * (fromPoint * fromPoint - fromPoint); //horrible kludge to fit curve to straight line
